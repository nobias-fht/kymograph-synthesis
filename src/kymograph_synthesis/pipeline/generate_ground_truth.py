    ...
    for p in range(particle_positions.shape[1]):
        for t in range(n_steps):
            ...
            indices = np.concatenate([
                ...
                np.arange(np.floor(x0), np.floor(x1), d1 if d1 != 0 else 1).astype(int),
                np.arange(np.floor(x1), np.floor(x2), d2 if d2 != 0 else 1).astype(int)
            ])
            in_bounds = np.logical_and((0 <= indices), (indices < n_spatial_values))
            kymograph_gt[t, indices[in_bounds], particle_states[t, p]] = 1    
            
    return GenerateGroundTruthOutput(kymograph_gt)